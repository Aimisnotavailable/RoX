...
class AR:
    ...
    def calculate_hand_points(self, landmarks, label, is_generated=False):
        ...
        for idx, lm in enumerate(raw_landmarks):
            ...
            sanitized = self._sanitize_point(x_px, y_px, self.W, self.H)
    ...
    def render_camera_feed(self, pos=(0,0)):
        if not self.image == None:
            surf.blit(pygame.transform.scale(self.image, surf.get_size()), pos)

    def update(self, frame):        
        if frame is None:
            get_logger_info('ERROR', f"Frame is a None Object")
            return
